...
# Safety: if still empty, default to PROD
...
if (-not $Yaml.AssetTagging) {
    throw "AssetTagging.yaml missing AssetTagging root"
}
    ...
    foreach ($queryItem in @($rule.Query)) {
        ...
        if ($queryEngine -eq 'DEFENDERGRAPH') {
            ...
            if ([string]::IsNullOrWhiteSpace($assetTagName)) {
                Write-Warn2 "query returned rows but AssetTagName is empty; skipping"
                continue
            }
        ...
        elseif ($queryEngine -eq 'AZURERESOURCEGRAPH') {
            ...
            # IMPORTANT CHANGE:
            # - Tag key MUST come from each row's AssetTagType column
            # - Tag value MUST come from each row's AssetTagName column (or fallback to YAML rule AssetTagName)
            ...
            # Extract distinct resource IDs (we’ll tag each individually)
            $resourceIds = @(
                foreach ($r in $rows) {
            ...
            foreach ($rid in $resourceIds) {
                ...
                # Match a row for this resourceId so we can read AssetTagType / AssetTagName
                ...
                # TagKey from output column AssetTagType
                $tagKey = $null
                ...
                # TagValue from output column AssetTagName (per-row), else fallback to YAML rule AssetTagName
                $tagValue = $null
                ...
                try {
                    ...
                    if ($result.Changed) {
                        Write-Ok ("Azure tag set: {0} -> {1}='{2}'" -f $nameHint, $tagKey, $tagValue)
                    }
                    else {
                        Write-Info ("Azure tag already present: {0} -> {1}='{2}'" -f $nameHint, $tagKey, $tagValue)
                    }
                }
                catch {
