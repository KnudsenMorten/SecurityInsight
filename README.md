...
flowchart TB
    ...
    subgraph RA ["2. Risk Analysis"]
        R1[119 attacker-centric KQL reports<br/>across Endpoint, Identity, Azure, PublicIP]
    end
...
| **risk-analysis** | `engine/risk-analysis/` | **119 reports** (59 Summary + 60 Detailed) across 4 security domains (Endpoint, Identity, Azure, PublicIP). EG-primary RA pattern: queries source from Microsoft Exposure Graph (nodes + edges), join `SI_*_Profile_CL` only for Tier / CMDB enrichment. 3-layer score model (RiskScoreTotal → RiskScore_Weight_Factor → RiskScoreTotal_Weighted). | `SI_RiskAnalysis_Summary_CL`, `SI_RiskAnalysis_Detailed_CL` |
