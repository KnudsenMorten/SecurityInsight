...
| 🔐 **Storage auth auto-detects OAuth** | Asset-profiling engine: when `$global:SI_StorageKey` is empty, defaults to OAuth (the v2.2.55 prestage grants Storage Blob/Table/Queue Data Contributor on the SA). Existing installs with a key still use SharedKey for back-compat. Force one or the other with `$global:SI_UseStorageOAuth = $true|$false`. Internal-vm launchers also honor the global from custom.ps1 — no `-UseStorageOAuth` CLI switch required. | v2.2.74 / v2.2.79 |
