...
**SecurityInsight** is a free, community-built add-on that consolidates security telemetry from Microsoft Defender, Entra ID, Active Directory, Azure, and Microsoft ExposureGraph. The goal: see risk like an attacker, respond like a defender. Every recommendation across Endpoint, Azure, Identity, and Public IPs is scored on four dimensions—**consequence**, **Tier 0–3 asset criticality**, **risk factors** (Internet Exposure, Verified Secret, Lateral Movement, ExploitSignals, and more), and a **customizable Risk Index**. Built by Microsoft MVP Morten Knudsen and open for community contributions.
