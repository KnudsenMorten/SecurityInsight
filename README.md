...
> Counts come from `DATA/SecurityInsight_IdentityTiering.json`, regenerated by `Build_Tier_Definitions_JSON_File` against the current Microsoft Graph + Azure role definitions. The raw catalogs (`Azure_Roles_Catalog` = 873, `EntraID_APIPermissions_Catalog` = 1,845) are also persisted in the same file for downstream consumers.
