...
## v2.2.385 — Risk Score KPI table Total row now matches per-domain row sums (fallback `SecurityDomain` derivation from report name), and the AI narrative section now states which template generated the current KPI/Top-50/AI snapshot + how old it is.

### Why

**KPI table arithmetic.** Customer report: a Detailed-template email showed
`Total: 71,278 / 8,218 critical / 58,054 high / 4,700 medium / 306 low` but the per-domain rows above it summed to only `1,122 / 53 / 482 / 281 / 306`. Every column except `Low` was off by orders of magnitude. Root cause: the per-domain rows only count rows whose `SecurityDomain` field matches one of the 4 canonical names (`Endpoint`, `Identity`, `Azure`, `PublicIP`); the Total row uses `$kpi.TotalRows` / `$kpi.SevCount` (unfiltered). Most YAML reports don't declare a top-level `SecurityDomain` so 98% of rows had a blank field and got tallied only in Total, never in per-domain. Operators couldn't tell whether the per-domain rows were a filter, a sample, or a bug.

**Snapshot source attribution.** v2.2.350 introduced a 24h cross-template cache: the headline KPI, the Top-50 risky assets list, and the AI narrative are all snapshots from the FIRST template to complete in the window (Summary or Detailed). The second template's email silently replays the cached snapshot. Operators reading a Detailed-template email at 14:00 couldn't tell whether they were reading content generated live at 14:00 OR content generated by the Summary template at 04:00 and reused. The data (`SourceTemplate`, `GeneratedAt`) was already in the cache; just not rendered.

### What changed

`engine/risk-analysis/Invoke-RiskAnalysis.ps1`:

- **Per-row `SecurityDomain` fallback** — new helper `_DomainFromReportName` in the KPI tally maps the per-row `Report` field to one of the 4 canonical domains using prefix and substring patterns (`Device_*` / `Endpoint_*` → `Endpoint`; `Identity_*` and identity-flavoured Attack_Paths → `Identity`; `Azure_*` and Azure-targeted Attack_Paths → `Azure`; `PublicIP_*` → `PublicIP`). Endpoint-specific Attack_Paths patterns (`public_ip_to_vm`, `device_with_high_severity_vulnerabilities`) are tested BEFORE the PublicIP prefix so `Public_IP_to_VM` correctly classifies as Endpoint (the VM is the target). When the per-row `SecurityDomain` field is blank, the engine derives a domain from the report name; populated fields still win. Result: per-domain row sums match the Total row cell sums exactly.
- **AI-section source attribution line** — new footer paragraph under the AI narrative (and the "AI not enabled" placeholder) states either:
  - **Cache won:** `Headline KPI + Top-50 + this narrative: generated by RiskAnalysis_Summary 6.4h ago (within 24h window -- this RiskAnalysis_Detailed run reused the snapshot). Re-runs of either template within 24h replay the same snapshot.`
  - **Live this run:** `Headline KPI + Top-50 + this narrative: generated live by this run (RiskAnalysis_Detailed). Next template run within 24h will replay this snapshot.`
- Source/age data comes from `$script:_CachedRAKPI.SourceTemplate` + `.GeneratedAt` (when cache won) or `$global:ReportTemplate` + `Get-Date` (when live). No new config knobs.

### Migration notes

- **Per-domain row counts will go UP after upgrade** on customers whose YAMLs don't declare report-level `SecurityDomain` — that's the fix landing. Total cells stay numerically unchanged. The "Other / unclassified" gap disappears.
- **Headline KPI score may shift slightly** for environments with significant unclassified-domain rows: the per-domain weighted average (`Endpoint × 0.30 + Identity × 0.30 + Azure × 0.20 + PublicIP × 0.20`) now uses ALL rows in each domain, not just the canonically-tagged subset. Magnitude of shift depends on how the previously-unclassified rows were distributed -- can be ±1 to ±5 points in either direction.
- **Long-term**: the engine fallback is a safety net. The right answer is still to declare `SecurityDomain` at the top of every YAML report. Files without it now get classified by name-pattern; files WITH it are unchanged (declared value still wins).
- **Source attribution** is purely additive (new paragraph, no removed text). All HTML/CSS is inline and email-client-safe. No effect on Excel attachment, JSON output, or LA ingest.

### Verification

`_DomainFromReportName` unit-tested before ship across 13 cases including all 7 Summary reports, 3 representative Attack_Paths variants, the prefix-conflict edge case (`Public_IP_to_VM` resolving to Endpoint not PublicIP), empty input, and unknown name (returns blank). 13/13 pass.
