...
if ($cfg.Flavour -eq 'Internal') {
    ...
    if (-not (Test-Path -LiteralPath $pdPath)) {
        throw ("platform-defaults.ps1 not found at {0}. Internal flavour requires it (port from v1's Automation-DefaultVariables.psm1). See bootstrap\Onboarding.txt step 3." -f $pdPath)
    }
    ...
    # the cert thumbprint + tenant + KV context globals get populated. Also
    # safety-net for legacy platform-defaults.ps1 files generated by older
    # Port-V1Platform.ps1 versions that didn't set this themselves.
