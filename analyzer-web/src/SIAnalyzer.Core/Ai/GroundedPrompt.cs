...
/// port of the retired PowerShell POC's grounded-prompt logic (the parts that are pure string logic;
...
public static class GroundedPrompt
{
    ...
    public static string BuildNlToKql(string question, IReadOnlyList<string> allowedTables)
    {
        ...
        return
            ...
            "- Name every source table explicitly. `union *`, a wildcard like `union SI_*`, an unscoped\n" +
            "  `search` and `find` are REJECTED - the guardrail whitelists SOURCES, so a query reading\n" +
            "  anything outside the list above is refused even if it also reads a listed table.\n" +
