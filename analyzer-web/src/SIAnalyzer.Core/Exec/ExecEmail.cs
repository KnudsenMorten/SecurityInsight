...
namespace SIAnalyzer.Core.Exec;
...
public static class ExecEmailRenderer
{
    ...
    public static ExecEmailMessage Render(
        ...
        // AUDIT #10: set when the workspace returned an INCOMPLETE result. The email is the one
        // surface a CIO reads without the app in front of them, so a partial read has to say so
        // in the mail itself - subject, HTML body and the plain-text twin.
        string? partialWarning = null)
    {
        ...
        var isPartial = !string.IsNullOrWhiteSpace(partialWarning);
        ...
        // The subject carries it too: a mail read on a phone lock screen must not report an
        // improvement that is really a truncated query.
        if (isPartial) subject = "[PARTIAL DATA] " + subject;
        ...
        // AUDIT #10: the partial-data warning sits ABOVE the score, in red, before any figure.
        if (isPartial)
        {
            sb.Append("<tr><td style=\"padding:14px 24px 0;\">")
              .Append("<div style=\"background:#fdecea;border:2px solid #cf222e;border-radius:8px;padding:12px 14px;font-size:14px;font-weight:700;color:#8b1a1a;\">")
              .Append(h.Encode(partialWarning!))
              .Append("</div></td></tr>");
        }
        ...
        return new ExecEmailMessage(subject, sb.ToString(), RenderText(headline, dashboard, execSummary, summaryFromAi, isLive, asOf, scoreText, boardDeckUrl, topRisks, actions, partialWarning));
    }
    ...
    private static string RenderText(
        ...
        IReadOnlyList<RiskRow> topRisks, IReadOnlyList<QuickWin> actions, string? partialWarning)
    {
        var sb = new StringBuilder();
        ...
        {
            sb.Append('\n').Append("*** ").Append(partialWarning!.Trim()).Append(" ***\n");
        }
