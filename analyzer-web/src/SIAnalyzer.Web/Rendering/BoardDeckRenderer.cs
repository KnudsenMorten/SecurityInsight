...
public static class BoardDeckRenderer
{
    ...
    /// <param name="nonce">Per-request CSP nonce (audit #13). Null renders a plain script tag,
    /// which is correct only for an artifact that is not served by us.</param>
    public static string Render(ExecViewModel vm, string? nonce = null)
    {
        var d = vm.Dashboard;
        var hl = d.Headline;
...
""");
        ...
        // AUDIT #10: the very first thing on the page, ABOVE the header - the header carries the
        // big score badge, and on a printed handout that number is what a reader takes in first.
        // The deck is circulated without the app in front of anyone, so the caveat cannot come
        // after the figure it qualifies.
        if (vm.Quality?.IsPartial == true)
        {
            sb.Append("<section class=\"partial-banner\" role=\"alert\"><strong>")
        ...
        // AUDIT #13: was onclick="window.print()" -- an inline handler no nonce can whitelist.
        sb.Append("<div class=\"no-print toolbar\"><button type=\"button\" data-print=\"1\" class=\"btn\">Print / Save as PDF</button> ");
        sb.Append("<a href=\"/exec\" class=\"btn ghost\">Back to dashboard</a></div>");

        sb.Append("</main>");
        // This deck had NO script at all before; the print button now needs one.
        sb.Append(ExecHtmlRenderer.ScriptTag(ExecHtmlRenderer.PrintInit, nonce));
        sb.Append("</body></html>");
        return sb.ToString();
    }
...
.partial-banner{font-size:15px;margin:14px 0;padding:12px 16px;border-radius:10px;background:#fdecea;border:2px solid #cf222e;color:#8b1a1a}
