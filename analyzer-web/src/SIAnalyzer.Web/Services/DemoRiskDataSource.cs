...
public sealed class DemoRiskDataSource : IRiskDataSource
{
    ...
    /// <summary>The seed is a local file, so a demo read is always complete by construction -
    /// there is no workspace to return a partial result.</summary>
    public Task<RiskSnapshot> GetAllRowsAsync(CancellationToken ct = default) =>
        Task.FromResult(new RiskSnapshot(_rows, DataQuality.Complete(DateTimeOffset.UtcNow)));
    ...
    public Task<QueryResult> RunGuardedQueryAsync(string kql, CancellationToken ct = default)
    {
        ...
        var latest = SIAnalyzer.Core.Analysis.SnapshotDiff.LatestSnapshot(_rows);
