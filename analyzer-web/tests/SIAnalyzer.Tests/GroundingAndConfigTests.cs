...
public sealed class GroundingAndConfigTests
{
    ...
    {
        var text = GroundedPrompt.TemplatedSummary(Array.Empty<SIAnalyzer.Core.Model.RiskRow>(), Audience.Management);
    ...
    public void No_workspace_falls_back_to_demo_but_flags_the_gap()
    {
        ...
        Assert.True(r.UseDemoData);
        Assert.Contains("Set SIAnalyzer:WorkspaceId", r.Source);
    }

    private static IReadOnlyList<SIAnalyzer.Core.Model.RiskRow> SnapshotRows() =>
        SnapshotDiffLatest(DemoData.Load(TestData.SeedPath()));

    private static IReadOnlyList<SIAnalyzer.Core.Model.RiskRow> SnapshotDiffLatest(IReadOnlyList<SIAnalyzer.Core.Model.RiskRow> rows) =>
        SIAnalyzer.Core.Analysis.SnapshotDiff.LatestSnapshot(rows);
}
