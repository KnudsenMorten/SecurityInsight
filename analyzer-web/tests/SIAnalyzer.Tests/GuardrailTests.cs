...
public sealed class GuardrailTests
{
    ...
    [Fact]
    public void Rejects_an_unrecognised_table_in_a_source_position()
    {
        // A non-_CL table used to be invisible to the guardrail's regex scan - neither
        // allow-listed nor rejected. Since audit #6 it is caught by SOURCE POSITION, so the
        // name no longer has to look like an SI table to be refused (KqlSourceWhitelistTests).
