...
public sealed class McpTests
{
    ...
    {
        // AUDIT #11: this test used to assert every description says "read-only" - and it PASSED,
        // because send_exec_summary_email's description contained the words "(read-only)" while
        // the tool sent mail. A test can only be as honest as the contract it checks. The contract
        // is now "read-only EXCEPT one declared side-effecting tool", so the assertion splits.
        foreach (var t in McpServer.Tools)
        {
            ...
            if (t.HasSideEffects)
            {
                Assert.Contains("SIDE EFFECT", t.Description, StringComparison.Ordinal);
            }
            else
            {
        ...
        // Exactly one side-effecting tool exists, and the DEFAULT catalogue contains none.
        Assert.Single(McpServer.Tools.Where(t => t.HasSideEffects));
        Assert.All(McpServer.ToolsFor(allowEmailSend: false), t => Assert.False(t.HasSideEffects));
    }
    ...
    public async Task Send_exec_summary_email_tool_is_fail_soft_with_no_recipients()
    {
        ...
        var resp = await McpServer.HandleAsync(req, svc, default, email, allowEmailSend: true);
        Assert.Null(resp!["error"]);
        var text = resp!["result"]!["content"]![0]!["text"]!.GetValue<string>();
    ...
    {
        // Enabled but with no email service wired => the tool reports unavailable, never throws.
        var req = new JsonObject
        {
            ["jsonrpc"] = "2.0",
            ["id"] = 12,
    ...
    private sealed class OfflineAi : IAiNarrativeService
    {
        ...
        public Task<NarrativeResult> SummarizeAsync(string instruction, IReadOnlyList<SIAnalyzer.Core.Model.RiskRow> rows, SIAnalyzer.Core.Ai.Audience audience, SIAnalyzer.Core.Ai.DiffSummary? diff = null, CancellationToken ct = default)
            => Task.FromResult(new NarrativeResult(SIAnalyzer.Core.Ai.GroundedPrompt.TemplatedSummary(rows, audience, diff), false));
