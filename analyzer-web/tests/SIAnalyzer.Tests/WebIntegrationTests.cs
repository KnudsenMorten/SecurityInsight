...
/// </summary>
public sealed class WebIntegrationTests : IClassFixture<SIAnalyzerAppFactory>
{
    private readonly SIAnalyzerAppFactory _factory;
    public WebIntegrationTests(SIAnalyzerAppFactory factory) => _factory = factory;
    ...
    [Fact]
    public async Task Mcp_endpoint_does_not_list_the_email_tool_by_default()
    {
        // AUDIT #11: the agent-facing surface is read-only unless an operator opts in. The tool
        // still exists for humans (the UI, POST /api/email/send, the scheduler) - the enabled case
        // is covered by McpSideEffectTests.
        var client = _factory.CreateClient();
        var resp = await client.PostAsJsonSafeAsync("/mcp", new { jsonrpc = "2.0", id = 1, method = "tools/list" });
        var body = await resp.Content.ReadAsStringAsync();
        Assert.DoesNotContain("send_exec_summary_email", body);
        Assert.Contains("guarded_query", body);          // the read-only surface is intact
    }
...
public sealed class SIAnalyzerAppFactory : WebApplicationFactory<Program>
{
