...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for PlatformConfiguration\Enable-VmManagedIdentity.
.DESCRIPTION
...
Write-Banner -Solution 'PlatformConfiguration' -Engine 'Enable-VmManagedIdentity' -Flavour 'community-azure' -Description 'Enable-VmManagedIdentity -- v2 ported engine under PlatformConfiguration.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'Enable-VmManagedIdentity.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'Enable-VmManagedIdentity.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
