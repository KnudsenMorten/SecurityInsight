...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for PlatformConfiguration\New-SelfSignedCertificate.
.DESCRIPTION
...
Write-Banner -Solution 'PlatformConfiguration' -Engine 'New-SelfSignedCertificate' -Flavour 'community-azure' -Description 'New-SelfSignedCertificate -- v2 ported engine under PlatformConfiguration.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'New-SelfSignedCertificate.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'New-SelfSignedCertificate.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
