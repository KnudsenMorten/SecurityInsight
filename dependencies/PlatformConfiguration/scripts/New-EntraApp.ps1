...
if (-not $global:SpnTenantId -or [string]::IsNullOrWhiteSpace([string]$global:SpnTenantId)) {
    throw "PlatformConfiguration\\New-EntraApp: `$global:SpnTenantId not set. Configure the bootstrap SPN in LauncherConfig.ps1 first."
}
...
if (-not $SkipSecretCreation) {
    ...
    $pwdCred = Add-MgApplicationPassword -ApplicationId $app.Id -PasswordCredential @{
        DisplayName = "Created by PlatformConfiguration on $((Get-Date).ToString('yyyy-MM-dd'))"
        EndDateTime = $endDate
    }
