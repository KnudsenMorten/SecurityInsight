...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for PlatformMonitoring\Invoke-PlatformHealthCheck.
.DESCRIPTION
    Runs the Invoke-PlatformHealthCheck engine from an Azure host that has a system-assigned MI.
...
Write-Banner -Solution 'PlatformMonitoring' -Engine 'Invoke-PlatformHealthCheck' -Flavour 'community-azure' -Description 'Platform-wide health-check engine. Runs a configurable battery of checks and sends email alerts on failure.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'Invoke-PlatformHealthCheck.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'Invoke-PlatformHealthCheck.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
