...
<#
.SYNOPSIS
    Community VM launcher for PlatformMonitoring\Invoke-PlatformHealthCheck.
.DESCRIPTION
    Runs the Invoke-PlatformHealthCheck engine on a Windows box in the customer's own tenant.
...
Write-Banner -Solution 'PlatformMonitoring' -Engine 'Invoke-PlatformHealthCheck' -Flavour 'community-vm' -Description 'Platform-wide health-check engine. Runs a configurable battery of checks and sends email alerts on failure.'
