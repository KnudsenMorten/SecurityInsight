...
.SYNOPSIS
    ...
    Validate-SILogAnalytics.ps1 so ingestion engines (RiskAnalysis,
...
function Ensure-SecurityInsightRg {
    ...
    if (-not $rg) {
        ...
        Write-Host "[INFO] RG exists: $ResourceGroup ($($rg.Location))" -ForegroundColor White
    }
...
function Ensure-SecurityInsightDce {
    ...
    if ($rgChanged) {
        Write-Host "[INFO] Waiting 60s for RBAC propagation..." -ForegroundColor White
