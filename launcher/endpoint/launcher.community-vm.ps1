...
<#
.SYNOPSIS
    Community VM launcher for SecurityInsight asset-profiling endpoint engine.
    Invokes Invoke-SIEngineRun -Engine endpoint.

.DESCRIPTION
    Runs the SecurityInsight asset-profiling endpoint engine on a Windows box in
...
.NOTES
    ...
    Engine         : asset-profiling/endpoint (Invoke-SIEngineRun -Engine endpoint)
...
Write-Banner -Solution 'SecurityInsight' -Engine 'AssetProfiling_Endpoint' -Flavour 'community-vm' -Version $versionStamp
$global:SI_TranscriptPath = Start-SILauncherTranscript -Engine 'endpoint' -Flavour 'community-vm' -RepoRoot $InstallPath
...
# Resolve Sinks: CLI > $global:SI_Sinks_Endpoint > engine default.
if ($cliBound.ContainsKey('Sinks') -and $Sinks) {
    $effectiveSinks = $Sinks
} elseif ($global:SI_Sinks_Endpoint) {
    $effectiveSinks = @($global:SI_Sinks_Endpoint)
} else {
    $effectiveSinks = @('LA','JSON','Excel')
}

# Resolve AssetLimit: CLI > $global:SI_AssetLimit_Endpoint > 0.
if ($cliBound.ContainsKey('AssetLimit')) {
    $effectiveAssetLimit = [int]$AssetLimit
} elseif ($null -ne $global:SI_AssetLimit_Endpoint) {
    $effectiveAssetLimit = [int]$global:SI_AssetLimit_Endpoint
} else {
    $effectiveAssetLimit = 0
}

# Resolve ForceFullRun: CLI > $global:SI_ForceFullRun_Endpoint > $global:SI_ForceFullRun.
$effectiveForceFullRun = $false
if ($cliBound.ContainsKey('ForceFullRun')) {
    $effectiveForceFullRun = [bool]$ForceFullRun
} elseif ($global:SI_ForceFullRun_Endpoint) {
    $effectiveForceFullRun = [bool]$global:SI_ForceFullRun_Endpoint
...
try {
    ...
    # Launcher lives at v2.2/launcher/endpoint/. Engine entry is at
    ...
    Write-Info "engine: $engine -Engine endpoint"
    & $engine -Engine 'endpoint' -Sinks $effectiveSinks @cliPassthrough
