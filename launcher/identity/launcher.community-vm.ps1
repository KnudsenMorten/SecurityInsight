...
<#
.SYNOPSIS
    Community VM launcher for SecurityInsight asset-profiling identity engine.
    Invokes Invoke-SIEngineRun -Engine identity.

.DESCRIPTION
    Runs the SecurityInsight asset-profiling identity engine on a Windows box in
...
Write-Banner -Solution 'SecurityInsight' -Engine 'AssetProfiling_Identity' -Flavour 'community-vm' -Version $versionStamp
$global:SI_TranscriptPath = Start-SILauncherTranscript -Engine 'identity' -Flavour 'community-vm' -RepoRoot $InstallPath
