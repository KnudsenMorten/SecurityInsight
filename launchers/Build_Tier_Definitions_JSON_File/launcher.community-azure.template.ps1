...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for SecurityInsight\Build_Tier_Definitions_JSON_File.
.DESCRIPTION
    Runs the Build_Tier_Definitions_JSON_File engine from an Azure host that has a system-assigned MI.
...
Write-Banner -Solution 'SecurityInsight' -Engine 'Build_Tier_Definitions_JSON_File' -Flavour 'community-azure' -Description 'Build_Tier_Definitions_JSON_File -- v2 ported engine under SecurityInsight.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'Build_Tier_Definitions_JSON_File.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'Build_Tier_Definitions_JSON_File.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
