...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for SecurityInsight\CriticalAssetTagging.
.DESCRIPTION
    Runs the CriticalAssetTagging engine from an Azure host that has a system-assigned MI.
...
Write-Banner -Solution 'SecurityInsight' -Engine 'CriticalAssetTagging' -Flavour 'community-azure' -Description 'CriticalAssetTagging -- v2 ported engine under SecurityInsight.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'CriticalAssetTagging.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'CriticalAssetTagging.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
