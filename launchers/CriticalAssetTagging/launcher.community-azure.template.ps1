...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\CriticalAssetTagging.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'CriticalAssetTagging (community cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\SecurityInsight\SCRIPTS\CriticalAssetTagging.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'CriticalAssetTagging (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "CriticalAssetTagging FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
