...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\CriticalAssetTaggingMaintenance.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'CriticalAssetTaggingMaintenance (community cloud) starting'
...
try {
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'CriticalAssetTaggingMaintenance (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "CriticalAssetTaggingMaintenance FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
