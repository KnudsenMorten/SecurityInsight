...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for SecurityInsight\CriticalAssetTaggingMaintenance_FixConflictingTags.
.DESCRIPTION
    Runs the CriticalAssetTaggingMaintenance_FixConflictingTags engine from an Azure host that has a system-assigned MI.
...
Write-Banner -Solution 'SecurityInsight' -Engine 'CriticalAssetTaggingMaintenance_FixConflictingTags' -Flavour 'community-azure' -Description 'CriticalAssetTaggingMaintenance_FixConflictingTags -- v2 ported engine under SecurityInsight.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'CriticalAssetTaggingMaintenance_FixConflictingTags.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'CriticalAssetTaggingMaintenance_FixConflictingTags.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
