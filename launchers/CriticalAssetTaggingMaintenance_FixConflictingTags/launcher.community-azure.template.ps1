...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\CriticalAssetTaggingMaintenance_FixConflictingTags.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'CriticalAssetTaggingMaintenance_FixConflictingTags (community cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\SecurityInsight\SCRIPTS\CriticalAssetTaggingMaintenance_FixConflictingTags.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'CriticalAssetTaggingMaintenance_FixConflictingTags (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "CriticalAssetTaggingMaintenance_FixConflictingTags FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
