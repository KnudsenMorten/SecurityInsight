...
<#
.SYNOPSIS
    Community Azure (Function / Logic App / Hybrid Worker) launcher for SecurityInsight\Deploy_OpenAI_PAYG_Instance_SecurityInsights.
.DESCRIPTION
    Runs the Deploy_OpenAI_PAYG_Instance_SecurityInsights engine from an Azure host that has a system-assigned MI.
...
Write-Banner -Solution 'SecurityInsight' -Engine 'Deploy_OpenAI_PAYG_Instance_SecurityInsights' -Flavour 'community-azure' -Description 'Deploys (or reuses) an Azure OpenAI PAYG account + model deployment used by SecurityInsight for AI summary generation. Customer values (SubscriptionId, ResourceGroupName, Location, AccountName, DeploymentName) are set via launcher.override.ps1 (sibling, .gitignore''d) or edited directly in the launcher template.'
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'Deploy_OpenAI_PAYG_Instance_SecurityInsights.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) { throw "Launcher: engine 'Deploy_OpenAI_PAYG_Instance_SecurityInsights.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
