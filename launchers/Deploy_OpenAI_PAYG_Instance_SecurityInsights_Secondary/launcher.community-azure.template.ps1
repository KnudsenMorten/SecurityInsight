...
<#
.SYNOPSIS
    Community cloud launcher for Deploy_OpenAI (Secondary) -- external user in a Function/LogicApp with their own MI + KV holding Modern-*-Azure.
#>
[CmdletBinding()]
...
Connect-AzAccount -ServicePrincipal -TenantId $ctx.Tenant.Id -Credential $cred -Subscription '<secondary-subscription-id>' | Out-Null
