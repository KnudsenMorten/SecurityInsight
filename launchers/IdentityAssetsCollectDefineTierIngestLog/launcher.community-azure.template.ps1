...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\IdentityAssetsCollectDefineTierIngestLog.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'IdentityAssetsCollectDefineTierIngestLog (community cloud) starting'

try {
    $engine = Join-Path $InstallPath 'SOLUTIONS\SecurityInsight\SCRIPTS\IdentityAssetsCollectDefineTierIngestLog.ps1'
    if (-not (Test-Path -LiteralPath $engine)) { throw "Cloud launcher: engine script not found at $engine." }
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'IdentityAssetsCollectDefineTierIngestLog (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "IdentityAssetsCollectDefineTierIngestLog FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
