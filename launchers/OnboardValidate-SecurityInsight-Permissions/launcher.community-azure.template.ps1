...
<#
.SYNOPSIS
    Community Azure launcher for SecurityInsight\OnboardValidate-SecurityInsight-Permissions.
.DESCRIPTION
    Thin wrapper around the OnboardValidate-SecurityInsight-Permissions.ps1
...
Write-Banner -Solution 'SecurityInsight' -Engine 'OnboardValidate-SecurityInsight-Permissions' -Flavour 'community-azure' -Version $versionStamp
...
foreach ($case in 'SCRIPTS','scripts') {
    $candidate = Join-Path $engineOwner (Join-Path $case 'OnboardValidate-SecurityInsight-Permissions.ps1')
    if (Test-Path -LiteralPath $candidate) { $engine = $candidate; break }
}
if (-not $engine) {
    throw "Launcher: engine 'OnboardValidate-SecurityInsight-Permissions.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts."
}
