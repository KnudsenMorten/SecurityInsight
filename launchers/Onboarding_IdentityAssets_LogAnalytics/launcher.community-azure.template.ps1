...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\Onboarding_IdentityAssets_LogAnalytics.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Onboarding_IdentityAssets_LogAnalytics (community cloud) starting'
...
try {
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Onboarding_IdentityAssets_LogAnalytics (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Onboarding_IdentityAssets_LogAnalytics FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
