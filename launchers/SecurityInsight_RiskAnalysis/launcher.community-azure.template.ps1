...
<#
.SYNOPSIS
    Community cloud-native launcher for SecurityInsight\SecurityInsight_RiskAnalysis.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'SecurityInsight_RiskAnalysis (community cloud) starting'
...
try {
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'SecurityInsight_RiskAnalysis (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "SecurityInsight_RiskAnalysis FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
