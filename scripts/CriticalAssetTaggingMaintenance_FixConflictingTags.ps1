...
################################################################
# FIX CONFLICTING TAGS from all devices (Defender)
################################################################
# ── Build the KQL query (no hardcoded tag — query finds all wrong tier tags) ──
$Query = @'
ExposureGraphNodes
...
| where AssetTags has "--SI"
| extend AssetTagsArray = split(AssetTags, ";")
| mv-apply tag = AssetTagsArray to typeof(string) on (
    summarize TierTags = make_list_if(tag, tag matches regex @"--tier\d+--SI")
)
| where array_length(TierTags) > 1
| extend TierTags          = array_sort_asc(TierTags)
| extend WinningTierTag    = tostring(TierTags[0])
| extend WinningTierNumber = toint(extract(@"--tier(\d+)--SI", 1, WinningTierTag))
| mv-expand LosingTag = TierTags to typeof(string)
| extend LosingTierNumber = toint(extract(@"--tier(\d+)--SI", 1, tostring(LosingTag)))
| where LosingTierNumber > WinningTierNumber
| project
    NodeId,
    NodeName,
    NodeLabel,
    ...
    AzureResourceId,
    WinningTierTag,
    TagToRemove = tostring(LosingTag),
    AllTierTags = strcat_array(TierTags, ";")
| order by NodeName asc, TagToRemove asc
'@
...
# ── Group rows by TagToRemove so we can bulk-remove one tag at a time ──
$tagGroups = $rows |
...
foreach ($group in $tagGroups) {
    ...
    Write-Info "Processing tag to remove: '$TagToDelete'"
    ...
    if ($deviceInventoryIds.Count -eq 0) {
        Write-Warn2 "No DeviceInventoryId values found for tag '$TagToDelete'; skipping"
        continue
    }

    # ── Lookup for NodeName/NodeLabel logging ──
    $rowsById = @{}
    ...
    for ($i = 0; $i -lt $deviceInventoryIds.Count; $i += $chunkSize) {
        ...
        # ── Log each affected server before bulk removal ──
        foreach ($id in $chunk) {
            if ($rowsById.ContainsKey($id)) {
                $r = $rowsById[$id]
                Write-Host "--------------------------------------------------------------------------"
                Write-Host ""
                Write-Info ("  Server  : {0}" -f $r.NodeName)
                Write-Info ("  Label   : {0}" -f $r.NodeLabel)
                Write-Info ("  Keep    : {0}" -f $r.WinningTierTag)
                Write-Info ("  Remove  : {0}" -f $TagToDelete)
                Write-Host ""
            }
        }
        ...
        try {
            ...
            Write-Ok ("Tag '$TagToDelete' removed from {0} machines (chunk {1}/{2})" -f $chunk.Count, $currentChunk, $totalChunks)
        }
        catch {
            Write-Err2 ("Bulk removal failed (chunk {0}/{1}): {2}" -f $currentChunk, $totalChunks, $_.Exception.Message)
            Write-Host ""
            Write-Info "Falling back to individual removal"

            foreach ($id in $chunk) {
                $nodeName  = if ($rowsById.ContainsKey($id)) { $rowsById[$id].NodeName  } else { $id }
                $nodeLabel = if ($rowsById.ContainsKey($id)) { $rowsById[$id].NodeLabel } else { "?" }
                try {
                    Remove-DefenderTag -AccessHeaders $AccessHeaders -DeviceInventoryId $id -Tag $TagToDelete | Out-Null
                    Write-Ok ("Tag '$TagToDelete' removed: $nodeName [$nodeLabel] ($id)")
                }
                catch {
                    Write-Err2 ("Failed removing tag from $nodeName [$nodeLabel] ($id): $($_.Exception.Message)")
                }
            }
        }
    }
