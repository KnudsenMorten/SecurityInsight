...
Write-Host ("    API permissions      : {0} granted" -f $apiCount)                                        -ForegroundColor White
Write-Host ("    Azure RBAC (subs)    : {0} granted / {1} failed / {2} skipped" -f $rbacGranted, $rbacFailed, $rbacSkipped) -ForegroundColor White
if ($defenderStatus) { Write-Host ("    Defender workspace   : {0}  ({1})" -f $defenderStatus.Status, $defenderStatus.Detail) -ForegroundColor White }
else                 { Write-Host ("    Defender workspace   : SKIP  (DefenderWorkspaceResourceId not provided)")            -ForegroundColor White }
if ($dcrStatus)      { Write-Host ("    DCR grant            : {0}  ({1})" -f $dcrStatus.Status, $dcrStatus.Detail)           -ForegroundColor White }
else                 { Write-Host ("    DCR grant            : SKIP  (DcrResourceId not provided)")                          -ForegroundColor White }
...
Write-Host ("    1. Re-run this script with -SpnAppId {0} to confirm no drift." -f $targetSp.appId) -ForegroundColor White
Write-Host  "    2. Run any SecurityInsight engine once (IAC, RiskAnalysis, ...) to ingest test data."                    -ForegroundColor White
Write-Host  "    3. Verify in Log Analytics:"                                                                             -ForegroundColor White
Write-Host  "         SI_IdentityAssets_CL | summarize Count=count() by ObjectType"                                       -ForegroundColor White
Write-Host  "         SI_RiskAnalysis_Summary_CL | top 10 by RiskScoreTotal desc"                                         -ForegroundColor White
Write-Host ""
Write-Sep
